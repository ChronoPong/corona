...
local trail1touchJoint,trail2touchJoint, dot
...
function scene:create( event )
    ...
    local sceneGroup = self.view
       ...
       -- ball = display.newCircle(sceneGroup, 200,  200 , 20)
        ball=display.newImageRect(sceneGroup,"ball.png",40,40,40,40)
        --ball:toFront()
        physics.addBody( ball, "dynamic", {density=0, friction=0, bounce=1,radius=20  } )
        ...
        function makeObstacle(x,y)
            ...
            physics.addBody(obstacle,"static", {density=1000, friction=0, bounce=2 })
        end
        ...
        function upWallCollision( event )
                ...
                --animation--
                -- 1st image sheet
                   --[[ local sheetData1 = { width=300, height=300, numFrames=6, sheetContentWidth=1800, sheetContentHeight=300 }
                    local sheet1 = graphics.newImageSheet( "thunderSprt2.png", sheetData1 )

    
                    -- In your sequences, add the parameter 'sheet=', referencing which image sheet the sequence should use
                    local sequenceData = {
                                    { name="seq1", sheet=sheet1, start=1, count=6, time=300, loopCount=0 }
                                   
                                    }

                    local myAnimation = display.newSprite( sceneGroup,sheet1, sequenceData )
                    myAnimation.x = display.contentWidth/2 ; myAnimation.y = display.contentHeight/2
                    myAnimation:play()
                    timer.performWithDelay( 300, function() myAnimation:removeSelf( ) ;myAnimation=nil end )
                    ]]
     ...
     --get the direction and set the speed
       ...
       function drawLine( event )
            ...
            if (event.phase=="began") then
                 dot=display.newCircle(sceneGroup,event.xStart,event.yStart,3)
            end
