...
local receivedPacket,updateBall,ballJoint,sendBallUpdate
-- -------------------------------------------------------------------------------
...
function scene:create( event )
    ...
    -- CREATHE THE BALL
        ...
        --local ballJoint = physics.newJoint( "touch", ball, ball.x, ball.y )
    ...
    function drawLine( event )
        if ( event.phase == "ended") then
            ...
            paddleImage:addEventListener( "collision", function () sendBallUpdate()  print("sendinggg") end )
            physics.addBody( paddleImage, "static", {density=1, friction=0, bounce=1 } )
            transition.fadeIn( paddleImage, {time = 400, onComplete = function ()
                transition.fadeOut( paddleImage, {time =400 , onComplete = function ()
                    paddleImage:removeSelf( )
